 ...
 * jpeg.hpp
 *
 * Provides functions that interface with libjpeg or libjpeg-turbo to read or
 * write jpeg files to or from Image objects.
 *
 * Author: Amir Kamil <akamil@umich.edu> Date: 2024-03-30
 */
...
// EFFECTS: Returns whether or not filename ends with .jpg or .jpeg, ignoring
//          capitalization.
bool has_jpeg_extension(const std::string &filename);

// REQUIRES: image points to an Image object MODIFIES: *image, cout EFFECTS:
// Reads a data from the JPEG file specified by the given filename into the
// given Image object. Returns whether or not the file was successfully read.
bool read_jpeg(Image *image, const std::string &filename);

// REQUIRES: image points to a valid Image object MODIFIES: the file
// corresponding to the given filename, cout EFFECTS:  Writes the data in the
// given Image object into the file of the given name, in JPEG format.
// high_quality specifies whether to use low- or high-quality settings for JPEG
// compression, which trades off file size for image quality. Returns whether or
// not the file was successfully written.
...
//           Image_width(image) if grayscale is true, or 3 * Image_width(image)
//           if it is false; image points to a valid Image; 0 <= row <
//           Image_height(image) MODIFIES: *image EFFECTS:  Copies the pixels
//           from buffer into image at the given row. If grayscale is true, then
//           each element of buffer is the value for all three colors of a
//           pixel. Otherwise, the buffer is assumed to hold color values in RGB
//           "striped" format (i.e. RGBRGB...).
...
// REQUIRES: info_p points to a valid jpeg_decompress_struct, with the header
//           already read and decompression started; image points to a valid
//           Image of the appropriate size MODIFIES: *image EFFECTS:  Reads the
//           data from the image source represented by info_p into the given
//           Image.
...
// REQUIRES: image points to a valid Image of the appropriate size; buffer
//           refers to an array that is at least as large as 3 *
//           Image_width(image); 0 <= row < Image_height(image) MODIFIES: the
//           contents of buffer EFFECTS:  Reads a full row from image into
//           buffer. Each color value is truncated by the value of Mask.
...
// REQUIRES: info_p points to a valid jpeg_compress_struct, with the metadata
//           already set and compression started; image points to a valid Image
//           of the appropriate size MODIFIES: the data target represented by
//           info_p EFFECTS:  Writes the data from the given Image to the data
//           target represented by info_p.
...
// MODIFIES: cout EFFECTS:  Prints an error message with instructions of how to
