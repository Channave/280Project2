...
// REQUIRES: img points to a valid Image MODIFIES: *img EFFECTS:  The image is
// rotated 90 degrees to the left (counterclockwise).
void rotate_left(Image *img);

// REQUIRES: img points to a valid Image. MODIFIES: *img EFFECTS:  The image is
// rotated 90 degrees to the right (clockwise).
...
// REQUIRES: energy points to a valid Matrix. cost points to a Matrix. energy
//           and cost aren't pointing to the same Matrix MODIFIES: *cost
//           EFFECTS:  cost serves as an "output parameter". The Matrix pointed
//           to by cost is initialized to be the same size as the given energy
//           Matrix, and then the cost matrix is computed and written into it.
...
// vector filled with the column numbers for each pixel along the seam, starting
// with the lowest numbered row (top of image) and progressing to the highest
// (bottom of image). The length of the returned vector is equal to
// Matrix_height(cost). While determining the seam, if any pixels tie for lowest
// cost, the leftmost one (i.e. with the lowest column number) is used. See the
// project spec for details on computing the minimal seam.
